...
///
///
#[deprecated(note = "This version of QAGO is note checked. Please use GO instead.")]
#[derive(Debug)]
...
impl<'a, T: Problem> EOA for QAGO<'a, T> {
    fn run(&mut self) -> OptimizationResult {
        ...
        while iter < max_iter {
            ...
            for k in 0..n {
                copy_vector(&x[worse_index[ind[k]]].genes, &mut worst_x[k].genes, d);
                //  worst_x[*k] = (x[worse_index[*k]].clone());
            }
            ...
            for i in 0..n {
                ...
                dgap[0] = best_x
                    .genes
                    .iter()
                    .zip(better_x[i].genes.iter())
                    .fold(0.0f64, |sum, (a, b)| sum + (a * b));
                dgap[1] = better_x[i]
                    .genes
                    .iter()
                    .zip(normal_x[i].genes.iter())
                    .fold(0.0f64, |sum, (a, b)| sum + (a * b));
                dgap[2] = normal_x[i]
                    .genes
                    .iter()
                    .zip(worst_x[i].genes.iter())
                    .fold(0.0f64, |sum, (a, b)| sum + (a * b));
                dgap[3] = x[l1]
                    .genes
                    .iter()
                ...
                let sum1: f64 = lf
                    .iter()
                    .zip(ls.iter())
                    .fold(0.0f64, |sum, (lfa, lsb)| sum + (lfa * f64::ln(lfa / lsb)));
                let sum2: f64 = sf
                    .iter()
                    .zip(ls.iter())
                    .fold(0.0f64, |sum, (sfa, lsb)| sum + (sfa * f64::ln(sfa / lsb)));
                ...
                for j in 0..d {
                    if i1[i][j] {
                        if i2[i][j] {
                            ...
                            newx[i].genes[j] = x[i].genes[j]
                                + randv
                                    * ((x[r[i][j]].genes[j] - x[i].genes[j])
                                        + (x[rm].genes[j] - x[i].genes[j])); // + (x[i].genes[j]))
                        }
                    }
